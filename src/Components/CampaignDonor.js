...
const CampaignDonor = () => {
  ...
  const handleDonateClick = (campaign) => {
    // Retrieve the existing campaigns from local storage
    const donationProjects = JSON.parse(localStorage.getItem('donationProjects')) || [];
  
    // Add the selected campaign to the array
    donationProjects.push({
      title: campaign.title,
      country: campaign.country || "N/A",
      donationAmount: 0, // Default donation amount (user can edit on Charities page)
      donationDescription: campaign.description,
    });
  
    // Save the updated list to local storage
    localStorage.setItem('donationProjects', JSON.stringify(donationProjects));
  
    // Notify the user
    toast.success(`${campaign.title} has been added to your donation cart!`);
  };
  
  const sortedCampaigns = sortCampaigns(sortOption); // Re-sort based on the current option

  return (
    <div className="campaign-page1">
      <section className="campaign-header1">
        <h1 className="campaign-title3">Our Campaigns</h1>
        <p className="campaign-description1">
      ...
      <section className="campaign-list1">
        {sortedCampaigns.map((campaign) => {
          ...
          return (
            <div
              className={`campaign-card1 ${isCompleted ? 'completed' : ''}`}
              key={campaign.id}
            >
              {isCompleted && (
                <div className="campaign-card1-overlay">
              ...
              <div className="campaign-info1">
                <h3 className="campaign-title1">{campaign.title}</h3>
                <p className="campaign-description1">{campaign.description}</p>
                <div className="campaign-progress1">
